...
CVSS_vector_pattern = re.compile( r'(CVSS:\d+\.\d+/)?AV:[NALP]/AC:[LH]/PR:[NLH]/UI:[NR]/S:[UC]/C:[HLN]/I:[HLN]/A:[HLN]') #regex currently does not work needs to be improved working on it in future implications
...
service_map = {
    ...
    "ftp": {"port": "21", "proto": "tcp", "name": "FTP"},         
        ...
        if data.get("result") == "OK" and "data" in data:
            ...
            if cve_id in documents:
                ...
                if description:
                    ...
                    print(f"[DEBUG] Found description: {description[:100]}...")  # Show first 100 chars use to debug the parsing from the API from vulners
...
# Discord command to generate Snort rule
@bot.command()
async def snort(ctx, cve_id: str):
    ...
    if cve_description:
        ...
        await ctx.send(
            ...
            f"Generated Snort rule:\n```{snort_rule}```\nFor more details, visit: https://vulners.com/cve/{cve_id}" #add a else statement to make sure snort rules dont get generated for non network attack vectors now that parsing is working perfectly 
          )
    else:
